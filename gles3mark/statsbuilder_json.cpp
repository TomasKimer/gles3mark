...
JSONStatsBuilder& JSONStatsBuilder::BuildGLinfo() {
    ...
    limits["MAX_TEXTURE_SIZE"     ] = GLQuery::Get<GLint>(GL_MAX_TEXTURE_SIZE);      // Maximum size of a texture (min 2048)
    //GL_MAX_3D_TEXTURE_SIZE Maximum size of 3D texture supported (min 256) glGetIntegerv
    
    limits["MAX_RENDERBUFFER_SIZE"] = GLQuery::Get<GLint>(GL_MAX_RENDERBUFFER_SIZE); // Maximum width and height of renderbuffers supported (min 2048)
    
    std::vector<GLint> maxDims = GLQuery::Get<GLint>(GL_MAX_VIEWPORT_DIMS, 2);
    limits["MAX_VIEWPORT_DIMS"    ] = toStr(maxDims[0]) + "x" + toStr(maxDims[1]);   // Dimensions of the maximum supported viewport size
    
    limits["MAX_SAMPLES"          ] = GLQuery::Get<GLint>(GL_MAX_SAMPLES);           // Maximum number of samples supported for multisampling (min 4)
    limits["MAX_COLOR_ATTACHMENTS"] = GLQuery::Get<GLint>(GL_MAX_COLOR_ATTACHMENTS); // Maximum number of color attachments supported (min 4)
    limits["MAX_DRAW_BUFFERS"     ] = GLQuery::Get<GLint>(GL_MAX_DRAW_BUFFERS);      // Maximum active number of draw buffers supported (min 4)

    limits["MAX_VERTEX_ATTRIBS"   ] = GLQuery::Get<GLint>(GL_MAX_VERTEX_ATTRIBS);    // Maximum number of vertex attributes supported (min 16)
